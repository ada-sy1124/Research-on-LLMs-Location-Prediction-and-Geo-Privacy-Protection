...
def _get_end_index(total_len: int) -> int:
    end_value = get_env("YES_ID_RERUN2_END_INDEX", "")
    if not end_value:
        return total_len
    try:
...
def run():
    ...
    input_dataset_path = get_path("YES_ID_RERUN2_INPUT_DATASET_PATH", str(REPO_ROOT / "data" / "YES_id"))
    output_dir = get_path("YES_ID_RERUN2_OUTPUT_DIR", str(REPO_ROOT / "data" / "YES_id_new_prompt2"))

    start_index = get_int("YES_ID_RERUN2_START_INDEX", 0)
    buffer_size = get_int("YES_ID_RERUN2_BUFFER_SIZE", 30)
    sleep_seconds = get_int("YES_ID_RERUN2_SLEEP_SECONDS", 15)
    temperature = get_float("YES_ID_RERUN2_GEMINI_TEMPERATURE", 0.0)
    chunk_start_id = get_int("YES_ID_RERUN2_CHUNK_START_ID", 0)

    print(
        "Current params: "
        f"YES_ID_RERUN2_START_INDEX={start_index}, YES_ID_RERUN2_BUFFER_SIZE={buffer_size}, "
        f"YES_ID_RERUN2_SLEEP_SECONDS={sleep_seconds}, GEMINI_MODEL={gemini_model}, "
        f"YES_ID_RERUN2_INPUT_DATASET_PATH={input_dataset_path}, YES_ID_RERUN2_OUTPUT_DIR={output_dir}"
    )
    ...
    client = create_genai_client("YES_ID_RERUN2_GEMINI_API_KEY")
