...
def prepare_inputs(model, tokenizer, image_processor, image, baseline_image, prompt):
    ...
    baseline_tensor = process_images([baseline_image], image_processor, model.config)[0]
    device = model.device
    dtype = model.dtype
...
def teacher_forced_token_logps(model, prompt_inputs, answer_ids, image, temperature):
    ...
    def custom_forward(input_ids, images, image_size):
        inputs, _, _, _, inputs_embeds, labels, _ = (
    ...
    if not torch.isfinite(logits).all():
        raise FloatingPointError("non-finite logits in LLaVA teacher-forced coordinate logprob")
    ...
    if not torch.isfinite(token_logps).all():
        raise FloatingPointError("non-finite token logprob in LLaVA teacher-forced coordinate logprob")
    return token_logps
