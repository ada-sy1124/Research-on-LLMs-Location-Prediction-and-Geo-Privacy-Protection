...
from geo_pg_geo import haversine_km, parse_coordinate
from geo_pg_score import compose_masked_image, generate_coordinate_text
...
def deletion_distance(
    ...
    text, _, _, raw_text = generate_coordinate_text(
        adapter=adapter,
        model=model,
        tokenizer=tokenizer,
...
def evaluate_deletion(
    ...
    results = {}
    with torch.no_grad():
        for raw_percent in args.geo_eval_topk.split(","):
            ...
            random_results = []
            for _ in range(args.geo_random_repeats):
